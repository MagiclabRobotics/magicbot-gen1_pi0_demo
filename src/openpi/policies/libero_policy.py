...
def make_libero_example() -> dict:
    ...
    return {
        ...
        "observation/right_hand_image": np.random.randint(256, size=(224, 224, 3), dtype=np.uint8),
        "prompt": "do something",
    }
...
class LiberoInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        right_hand_image = _parse_image(data["observation/right_hand_image"])
...
class LiberoOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
        # Only return the first 7 dims.(changed to 8 dims: grasp)
