...
PI0_DATA_ROOT: str = "/data3/Pi0_humaniod_data"
full_assets_dir = os.path.join(PI0_DATA_ROOT, "norm_stats_data")
...
@dataclasses.dataclass(frozen=True)
class LeRobotLiberoHumanDataConfig(DataConfigFactory):
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Make inputs look like they come from the Libero environment
        repack_transform = _transforms.Group(
                ...
                _transforms.RepackTransform(
                    {
                        ...
                        "observation/right_hand_image": "right_hand_image",
                        "observation/state": "state",
                        "actions": "actions",
                        "prompt": "prompt",
                    }
...
class TrainConfig:
    ...
    assets_base_dir: str = os.path.join(PI0_DATA_ROOT, "norm_stats_data")
    # Base directory for checkpoints.
    checkpoint_base_dir: str = os.path.join(PI0_DATA_ROOT, "checkpoints")
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi0_fast_droid",
        model=pi0_fast.Pi0FASTConfig(action_dim=8, action_horizon=10),  # model=pi0_fast.Pi0FASTConfig(paligemma_variant="gemma_2b_lora"),
        data=SimpleDataConfig(
            assets=AssetsConfig(asset_id="droid"),
            data_transforms=lambda model: _transforms.Group(
    ...
    TrainConfig(
        name="pi0_magiclab_full_finetune",   
        model=pi0.Pi0Config(),
        data=LeRobotLiberoHumanDataConfig(

            repo_id="real_pnp_green_orange",
            assets=AssetsConfig(
                assets_dir=full_assets_dir,
                asset_id="real_pnp_green_orange",
            ),
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                prompt_from_task=True,
    ...
    TrainConfig(
        name="pi0_magiclab_low_mem_finetune",   
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotLiberoHumanDataConfig(
